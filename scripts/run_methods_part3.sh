#!/bin/bash
# Parallel Grid Search Script 3/3
# Runs final set of method combinations

set -e

echo "╔════════════════════════════════════════════════════════════════════╗"
echo "║  Grid Search Part 3/3: Methods 5-6 with all hyperparameter combos ║"
echo "╚════════════════════════════════════════════════════════════════════╝"
echo ""

BASE_CONFIG="configs/my_config.yaml"

# Part 3: slot:query_attn, slot:self_attn
METHODS=("slot:query_attn" "slot:self_attn")
SEEDS=(1 2 3)
...
EOF
    ...
    echo "[Part3] $selector + $fuser | seed=$seed, llm=$lambda_llm, mixup=$lambda_mixup, lr=$lr"
...
echo ""
echo "✓ Part 3 completed ($total experiments)"
