...
METHOD="selector_mlp"
# selector_mlp uses MLP selector; you can also override selector_type via --selector_type if needed
EPOCHS=${EPOCHS:-50}
SEEDS=(1 2 3)
      ...
      for llm_coeff in "${LAMBDA_LLM_NEGATIVES[@]}"; do
        for mixup_coeff in "${LAMBDA_MIXUP[@]}"; do
          for margin in "${MARGINS[@]}"; do
            ...
            "./run_training.sh" --no-config --method "$METHOD" --epochs "$EPOCHS" --lr "$lr" --batch_size "$bs" --seed "$s" \
